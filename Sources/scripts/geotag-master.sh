...
# geotag-master.sh — fast batch normalize (+TZ) then geotag with GPX
...
FROM_TZ=""
TO_TZ=""
TZ_TAG=""            # +HH:MM to write OffsetTime* (also used for geotime if set)
...
VERBOSE=0
DRYRUN=0
MAX_EXT_SECS=0       # seconds tolerance outside GPX ends for pool prefilter
...
Timezones Z:  UTC+3, +03:00, +3, -04:30, UTC, Z
Drift V:      3m, 45s, 1:30m  (choose ahead/behind; no +/-)

How it works (fast path)
------------------------
Per photo folder:
  1) (Default mode) Compute NET = (to − from) − ahead + behind.
     Shift DateTimeOriginal/CreateDate/ModifyDate by NET in one call.
     Write OffsetTime* (+HH:MM) in one call.
  2) Geotag:
     - Build geotime:  -geotime<${DateTimeOriginal}+TZ>
       TZ is --tz-tag if set, else --to-tz; in --geotag-only, if neither is set,
       fallback per-file to +${OffsetTimeOriginal}.
     - Apply ALL local GPX (every *.gpx in the folder) in one call.
     - Prefilter pool GPX by overlap with the folder's photo time range (± --max-ext),
       then apply each selected pool GPX in one call.
     - If a pool GPX caused any “image files updated/created” and --copy-matched is set,
       copy that GPX into the folder (unique name if needed).

Notes
-----
- Geotagging is **always applied**, even if GPS already exists.
- Pool prefilter avoids random GPX matches and speeds up processing.

Examples
--------
# Olympus case (normalize + geotag; camera UTC+3, actual UTC+5, camera ahead 3m)
geotag-master.sh \
  --photos /data/2025-07-28 --pool /data/Downloads \
  --from-tz UTC+3 --to-tz UTC+5 --drift-ahead 3m \
  --tz-tag +05:00 --copy-matched --verbose

# Phone photos (times already correct): geotag only
geotag-master.sh \
  --photos /data/2025-07-28 --pool /data/GPX \
...
tz_to_secs(){
  ...
  if [[ "$z" =~ ^([+-])([0-9]{1,2})(:([0-9]{1,2}))?$ ]]; then
    local s="${BASH_REMATCH[1]}" h="${BASH_REMATCH[2]}" m="${BASHREMATCH[4]:-0}"
...
drift_to_secs(){
  ...
  elif [[ "$v" =~ ^([0-9]+)s$ ]]; then         echo $(( 10#${BASHREMATCH[1]} ))
  elif [[ "$v" =~ ^([0-9]+)$ ]]; then          echo $(( 10#$v ))
  else die "Bad drift '$v' (use 3m, 45s, 1:30m)"; fi
}

# exiftool via argfile; options then files after "--"
et(){
...
# same as et() but capture output text
et_capture(){
  local tmp; tmp="$(mktemp)"
  {
    echo "-P"
...
# folder time range (UTC epoch) using a single exiftool call
folder_time_range(){
  local dir="$1" tz="$2"
  local out
  out="$(exiftool -q -q -n -T -DateTimeOriginal -OffsetTimeOriginal -- \
         ...
         2>/dev/null || true)"
  [[ -z "$out" ]] && return 1
  local INF=9999999999 min=$INF max=0
  while IFS=$'\t' read -r dto otz; do
    [[ -z "$dto" ]] && continue
    local iso="${dto/:/-}"; iso="${iso/:/-}"
    local use_tz="$tz"; [[ -z "$use_tz" ]] && use_tz="$otz"
    [[ -z "$use_tz" ]] && continue
    local epoch; epoch="$(date -ud "$iso $use_tz" +%s 2>/dev/null || true)"
    [[ -z "$epoch" ]] && continue
    (( epoch < min )) && min=$epoch
    (( epoch > max )) && max=$epoch
  done <<<"$out"
...
sum_updated_from_output(){ awk '/image files (updated|created)/{s+=$1} END{print s+0}'; }

# ---------------- compute NET & TZ strings ----------------
MATCH_TZ_STR=""   # used for geotime in batch mode
...
if [[ $GEOTAG_ONLY -eq 1 ]]; then
  if [[ -n "$TZ_TAG" ]]; then
    ...
    MATCH_TZ_STR=""  # will use per-file +${OffsetTimeOriginal} later
  fi
else
...
while IFS= read -r -d '' dir; do
  ...
  # Local GPX (apply all; always redo)
  shopt -s nullglob
  ...
  # Pool GPX (prefilter by folder time range; always redo)
  if [[ -n "$GPX_POOL" ]]; then
